...
architecture Behavioral of create_pdu is
    ...
    signal data_len_sig : integer range 0 to MAX_DATA_LEN := 0;                          -- Data length (number of nibble)
...
begin
    ...
    begin
        ...
        if rising_edge(clk) then
            if start = '1' and active = '0' then
                ...
                if(pdu_type = '0') then
                    ...
                    pdu_len_sig <= (SYNC_LEN_REQUEST) + (PROTOCOL_LEN) + (ADDR_LEN) + (to_integer(unsigned(data_len))) + (FCS_LEN) + (END_LEN);
                    -- Add sync field, protocol selector, address field, data field, placeholder for FCS, and end field
                   buffer_sig(index - 1 downto (index - (SYNC_LEN_REQUEST*4) - (PROTOCOL_LEN*4) - (ADDR_LEN*4) - (data_len_sig*4) - (FCS_LEN*4) - (END_LEN*4) )) <= SYNC_REQUEST & protocol_selector & address & data_in((MAX_DATA_LEN*4)-1 downto (MAX_DATA_LEN*4)-(data_len_sig*4)) & "00000000" & END_FIELD; 
                else
                    -- Compute total PDU_Response length
                    pdu_len_sig <= (SYNC_LEN_RESPONSE) + (data_len_sig) + (FCS_LEN) + (END_LEN);
                   -- Add sync field, address field, data field, placeholder for FCS, and end field
                    buffer_sig(index - 1 downto (index - (SYNC_LEN_RESPONSE*4) - (data_len_sig*4) - (FCS_LEN*4) - (END_LEN*4) )) <= SYNC_RESPONSE & data_in((MAX_DATA_LEN*4)-1 downto (MAX_DATA_LEN*4)-(data_len_sig*4)) & "00000000" & END_FIELD;
                end if;
                ...
                if nibble_index < (pdu_len_sig - FCS_LEN - END_LEN) then
                    ...
                    -- Finalize FCS insertion once all bytes are processed
                     if(pdu_type = '0') then
                        buffer_sig(index - (SYNC_LEN_REQUEST*4) - (PROTOCOL_LEN*4) - (ADDR_LEN*4) - (data_len_sig*4) - 1 downto index - (SYNC_LEN_REQUEST*4) - (PROTOCOL_LEN*4) - (ADDR_LEN*4) - (data_len_sig*4) - FCS_LEN*4) <= fcs_out;
                     else
                        buffer_sig(index - (SYNC_LEN_RESPONSE*4) - (data_len_sig*4) - 1 downto index - (SYNC_LEN_RESPONSE*4) - (data_len_sig*4) - FCS_LEN*4) <= fcs_out;
                     end if;
                    -- Assign output
                    active <= '0';
                    done_sig <= '1';
        ...
        if pdu_len_sig < FCS_LEN + END_LEN then
            ...
            pdu_len <= std_logic_vector(to_unsigned(pdu_len_sig - FCS_LEN - END_LEN, 8));
        end if;
    end process;
    data_len_sig <= to_integer(unsigned(data_len));
    done <= done_sig;
    pdu_out <= buffer_sig;
end Behavioral;